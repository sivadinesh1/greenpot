...
export default function BlogAdd({blogs,handleSnackOpen, onReloadBlogList ,onMode,router}) {
	...
	const onSubmit = async (formData, event) => {
		...
		let tagsarr = [];
		tags.forEach((e, i) => {
			if (selectedTag.includes(e.slug))
				tagsarr.push(Number(e.id));
		});

		let categoriessarr = [];
		categories.forEach((e, i) => {
			if (selectedCat.includes(e.slug))
				categoriessarr.push(Number(e.id));
		});
    ...
    <div>
			...
			<div style={{
				...
				<div {...getRootProps()} className={`${stylesd.dropzone} ${isDragActive ? stylesd.active : null}`}>
								...
								Drop Zone</div>
								<span>
								{uploadedFiles.length > 0 && <Button onClick={handleOpenDialog} variant='outlined'   style={{backgroundColor: '#FFFFFF', color:'#12824C' }}>
						Show Gallery
							</Button>}
								</span>
				</div>
				<div className={styles.rowGap}>
				<InputLabel id="demo-mutiple-chip-label" style= { {color: '#000000' }}>Category *</InputLabel>
										<Select
				...
				<div className={styles.rowGap}>
				<InputLabel id="demo-mutiple-chip-label" style= { {color: '#000000' }}>Tag *</InputLabel>
										<Select
											labelId="demo-mutiple-chip-label"
											id="demo-mutiple-chip"
											multiple
            ...
            <div>
            <Dialog classes={{ paper : classes.dialogPaper}} fullWidth={true} maxWidth='lg' open={openDialog} onClose={handleCloseDialog} aria-labelledby='max-width-dialog-title'>
