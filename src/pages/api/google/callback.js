...
const strategyOptions = {
  clientID:
    "364546487930-thq5mmbibddn7l4bdjhp4l2d2jr2bpjj.apps.googleusercontent.com",
...
async function create(data, done) {
  ...
  if (currentUser) {
    console.log('user already exist: ', currentUser.username);
    done(null, currentUser);
  } else {
    let temp = {}
    temp.fname = data.name.givenName,
      temp.email = data.emails[0].value,
      temp.socialmediaId = data.id
	...
	return new Promise (function (resolve, reject){
		...
		.then((data) => {
            db.one('INSERT INTO user_role(user_id, role_id) VALUES($1, $2) RETURNING id', [data.id, 1]).then((data)=>{
                console.log('new inserted user id--> ' + data.id); // print new user id;
...
//   )
  ...
  .get(
    ...
    async (req, res) => {
      console.log("calling after second auth.", req.user);
      let user = req.user
      const session = { ...user }
